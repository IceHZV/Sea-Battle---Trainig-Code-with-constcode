class ComputerScene extends Scene {
  ...
  update() {
    ...
    if (isEnd) {
      if (opponent.loser) {
        this.status.textContent = "Caramba! You won!";
      } else {
        this.status.textContent = "These waters are too steep for you";
      }
    ...
    if (this.playerTurn) {
      this.status.textContent = "Yo-ho-ho! Think better captain";
    } else {
      this.status.textContent = "Pirate attacks";
    }
  }
}
